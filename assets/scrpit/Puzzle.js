...
cc.Class({
    ...
    start: function(err) {
        ...
        if(player.isMap){
            cc.log("player.isMap:"+player.isMap)
            cc.log("player.x:"+player.xPosition)
            cc.log("player.y:"+player.yPosition)
            startPos = cc.p(player.xPosition, player.yPosition+32)
        }
    ...
    // 是否遇敌
    _isMeetEnemy(){
        // 遇到跳转场景保存人物坐标,没有遇到增加概率
        let r = Math.floor(Math.random()*100)
        cc.log("随机值:"+r)
        if(r<this._probability){
            player.xPosition = this._player.getPosition().x
            player.yPosition = this._player.getPosition().y
            cc.log("player.x:"+player.xPosition)
            cc.log("player.y:"+player.yPosition)
            player.isMap = true
            ...
            this._probability += 1
            var i = this.node.getChildByName("prob")
            var j = i.getComponent(cc.Label)
            j.string = "遇怪率:"+this._probability+"%\nplayer.x:"+this._player.getPosition().x+
            " player.y:"+this._player.getPosition().y+"\nmap.x:"+this.node.getPosition().x+
            " map.y:"+this.node.getPosition().y
        }
    },
